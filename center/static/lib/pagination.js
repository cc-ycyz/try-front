var Pagination = {
    ...
    createHtml: function (pageIndex, recordCount, pageSize) {
        ...
        html.push("<span class=\"total\">共" + recordCount + "条数据</span>");
        
        var haschange = []
        if (pageIndex > 0) {
            haschange.push("<span class=\"previous\"><a href=\"javascript:;\" page= " + (pageIndex - 1) + " data-rec=\"上一页\"></a></span>");
        } else {
            haschange.push("<span class=\"disable previous\"></span>");
        }
        if (pageCount <= showPageCount) {
            for (var i = 0; i < pageCount; i++) {
                if (pageIndex == i) {
                    haschange.push("<span class=\"current num\">" + (i + 1) + "</span>");
                } else {
                    haschange.push("<span class=\"num\"><a href=\"javascript:;\"  page=" + i + ">" + (i + 1) + "</a></span>");
                }
            }
            ...
            haschange.push("<span class=\"dots\">...</span>");
            haschange.push("<span class=\"num\"><a href=\"javascript:;\"  page=" + (pageCount - 1) + ">" + pageCount + "</a></span>");
        } else if (pageIndex >= pageCount - HalfPageCount - 1) {
            haschange.push("<span class=\"num\"><a href=\"javascript:;\" page='0'>" + 1 + "</a></span>");
            haschange.push("<span class=\"dots\">...</span>");
        ...
        if (pageIndex < pageCount - 1) {
            haschange.push("<span id=\"fnext\" class=\"next\"><a href=\"javascript:;\" page=" + (pageIndex + 1) + " data-rec=\"下一页\"></a></span>");
        } else {
            haschange.push("<span class=\"disable next \" id=\"fnext\"></span>");
        }
        
        haschange.push("<span class=\"page_jump\">跳转到</span><input id='pageInput' class='pageInput' oldpage='' maxlength='" + pageCount + "' type='text' ><span class=\"page_jumps\">页</span><button type='button' id='pagebtn' class='pagebtn'>GO</button>");
        var str = "<span id='left'>" + html.join("") + "</span>" + "<span id='right'>"+haschange.join("")+"</span>";
        return str;
    }, 
